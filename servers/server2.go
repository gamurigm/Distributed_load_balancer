...
func (s *server) ProcessRequest(ctx context.Context, req *pb.Request) (*pb.Response, error) {
	...
	start := time.Now()
	computationResult := heavyComputation(5000000)
	elapsed := time.Since(start)

	log.Printf("Servidor 2 procesó solicitud #%d en %s", requestCount, elapsed)

	return &pb.Response{Result: fmt.Sprintf("Server2 [%d]: Computo = %.2f, Tiempo = %s", requestCount, computationResult, elapsed)}, nil
}
